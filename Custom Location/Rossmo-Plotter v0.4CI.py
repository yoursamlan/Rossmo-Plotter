# v0.4CI -> Custom Input for Crime Locations
# (c) Amlan Saha Kundu, 2023
...
def Rossmo(i2, c1, RES_PREFIX, Dist, B, f, g):
    ...
    for cross in range(NB_CRIMES):
        ...
        draw.text((25, 25), "Created using Rossmoss-Plotter v0.4CI by Amlan Saha Kundu, 2023", fill=color, font=fontmain)
        param = " B: " + str(B) + "\n f : " + str(f) + "\n g: " + str(g)
...
def get_crime_locations(num_crimes, width, height, buffer_range):
    x_cor = []
    y_cor = []

    print("\nEnter the coordinates for the crime locations (x, y) separated by space:")
    for i in range(num_crimes):
        while True:
            try:
                x, y = map(int, input(f"Crime {i+1}: ").split())
                if (buffer_range <= x < width-buffer_range) and (buffer_range <= y < height-buffer_range):
                    x_cor.append(x)
                    y_cor.append(y)
                    break
                else:
                    print(f"Invalid coordinates! Please ensure both x and y are within {buffer_range}px from the image border.")
            except ValueError:
...
x_cor, y_cor = get_crime_locations(val, width, height, border_range)
...
print("Crime locations are saved in", coor_file_path)
...
# Display the image after renaming and moving it to the output folder
final_image = RES_PREFIX + ".bmp"
print("Image saved as...", final_image)
